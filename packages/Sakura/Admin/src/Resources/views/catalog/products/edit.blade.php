<x-admin::layouts>
    ...
    {!! view_render_event('application.admin.catalog.product.edit.before', ['product' => $product]) !!}

    <x-admin::form
        method="PUT"
        enctype="multipart/form-data"
    >
        {!! view_render_event('application.admin.catalog.product.edit.actions.before', ['product' => $product]) !!}
        ...
        {!! view_render_event('application.admin.catalog.product.edit.actions.after', ['product' => $product]) !!}

        <!-- body content -->
        {!! view_render_event('application.admin.catalog.product.edit.form.before', ['product' => $product]) !!}

        <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
            @foreach ($product->attribute_family->attribute_groups->groupBy('column') as $column => $groups)
                {!! view_render_event('application.admin.catalog.product.edit.form.column_' . $column . '.before', ['product' => $product]) !!}
                ...
                <div class="flex flex-col gap-2 {{ $column == 1 ? 'flex-1 max-xl:flex-auto' : 'w-[360px] max-w-full max-sm:w-full' }}">
                    @foreach ($groups as $group)
                        ...
                        @if (count($customAttributes))
                            {!! view_render_event('application.admin.catalog.product.edit.form.' . $group->code . '.before', ['product' => $product]) !!}
                            ...
                            <div class="box-shadow relative rounded bg-white p-4 dark:bg-gray-900">
                                ...
                                @foreach ($customAttributes as $attribute)
                                    {!! view_render_event('application.admin.catalog.product.edit.form.' . $group->code . '.controls.before', ['product' => $product]) !!}
                            ...
                            {!! view_render_event('application.admin.catalog.product.edit.form.' . $group->code . '.after', ['product' => $product]) !!}
                        @endif
                    @endforeach
                ...
                {!! view_render_event('application.admin.catalog.product.edit.form.column_' . $column . '.after', ['product' => $product]) !!}
            @endforeach
        </div>

        {!! view_render_event('application.admin.catalog.product.edit.form.after', ['product' => $product]) !!}
    </x-admin::form>

    {!! view_render_event('application.admin.catalog.product.edit.after', ['product' => $product]) !!}
</x-admin::layouts>
