...
<header class="sticky top-0 z-[10001] flex items-center justify-between border-b bg-white px-4 py-2.5 dark:border-gray-800 dark:bg-gray-900">
    <div class="flex items-center gap-1.5">
        ...
        <a href="{{ route('admin.dashboard.index') }}">
            ...
            @else
                <img
                    src="{{ request()->cookie('dark_mode') ? application_asset('images/dark-logo.svg') : application_asset('images/logo.svg') }}"
                    id="logo-image"
                    alt="{{ config('app.name') }}"
                />
            @endif
        ...
        <x-admin::dropdown position="bottom-{{ core()->getCurrentLocale()->direction === 'ltr' ? 'right' : 'left' }}">
            ...
            <x-slot:content class="!p-0">
                <div class="flex items-center gap-1.5 border border-b-gray-300 px-5 py-2.5 dark:border-gray-800">
                    <img
                        src="{{ url('cache/logo/application.png') }}"
                        width="24"
                        height="24"
                    />
            ...
            template: '#v-notifications-template',
                ...
                data() {
                    return {
                        ...
                        orderTypeMessages: {
                        {{ \Sakura\Sales\Models\Order::STATUS_PENDING }}: "@lang('admin::app.notifications.order-status-messages.pending')",
                        {{ \Sakura\Sales\Models\Order::STATUS_CANCELED }}: "@lang('admin::app.notifications.order-status-messages.canceled')",
                        {{ \Sakura\Sales\Models\Order::STATUS_CLOSED }}: "@lang('admin::app.notifications.order-status-messages.closed')",
                        {{ \Sakura\Sales\Models\Order::STATUS_COMPLETED }}: "@lang('admin::app.notifications.order-status-messages.completed')",
                        {{ \Sakura\Sales\Models\Order::STATUS_PROCESSING }}: "@lang('admin::app.notifications.order-status-messages.processing')",
                        {{ \Sakura\Sales\Models\Order::STATUS_PENDING_PAYMENT }}: "@lang('admin::app.notifications.order-status-messages.pending-payment')",
                        }
                    }
                },
        ...
        app.component('v-dark', {
            ...
            data() {
                return {
                    ...
                    logo: "{{ application_asset('images/logo.svg') }}",

                    dark_logo: "{{ application_asset('images/dark-logo.svg') }}",
                };
            },
