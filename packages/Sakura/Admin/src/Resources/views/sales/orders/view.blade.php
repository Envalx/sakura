<x-admin::layouts>
    ...
    <div class="grid">
        <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
            {!! view_render_event('application.admin.sales.order.title.before', ['order' => $order]) !!}
            ...
            {!! view_render_event('application.admin.sales.order.title.after', ['order' => $order]) !!}
    ...
    <div class="mt-5 flex-wrap items-center justify-between gap-x-1 gap-y-2">
        <div class="flex gap-1.5">
            {!! view_render_event('application.admin.sales.order.page_action.before', ['order' => $order]) !!}
            ...
            {!! view_render_event('application.admin.sales.order.page_action.after', ['order' => $order]) !!}
        </div>
        ...
        <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
            ...
            <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
                {!! view_render_event('application.admin.sales.order.left_component.before', ['order' => $order]) !!}
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    ...
                    <div class="grid">
                        @foreach ($order->items as $item)
                            {!! view_render_event('application.admin.sales.order.list.before', ['order' => $order]) !!}
                            ...
                            {!! view_render_event('application.admin.sales.order.list.after', ['order' => $order]) !!}
                        @endforeach
                    </div>
                ...
                {!! view_render_event('application.admin.sales.order.left_component.after', ['order' => $order]) !!}
            </div>

            <!-- Right Component -->
            <div class="flex w-[360px] max-w-full flex-col gap-2 max-sm:w-full">
                {!! view_render_event('application.admin.sales.order.right_component.before', ['order' => $order]) !!}
                    ...
                    <x-slot:content>
                        <div class="{{ $order->billing_address ? 'pb-4' : '' }}">
                            <div class="flex flex-col gap-1.5">
                                ...
                                {!! view_render_event('application.admin.sales.order.customer_full_name.after', ['order' => $order]) !!}

                                <p class="text-gray-600 dark:text-gray-300">
                                    {{ $order->customer_email }}
                                </p>

                                {!! view_render_event('application.admin.sales.order.customer_email.after', ['order' => $order]) !!}

                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('admin::app.sales.orders.view.customer-group') : {{ $order->is_guest ? core()->getGuestCustomerGroup()?->name : ($order->customer->group->name ?? '') }}
                                </p>

                                {!! view_render_event('application.admin.sales.order.customer_group.after', ['order' => $order]) !!}
                            </div>
                        </div>
                        ...
                        @if ($order->billing_address)
                            ...
                            <div class="{{ $order->shipping_address ? 'pb-4' : '' }}">
                                ...
                                {!! view_render_event('application.admin.sales.order.billing_address.after', ['order' => $order]) !!}
                            </div>
                        @endif
                        ...
                        @if ($order->shipping_address)
                            ...
                            {!! view_render_event('application.admin.sales.order.shipping_address.after', ['order' => $order]) !!}
                        @endif
                    </x-slot>
                </x-admin::accordion>
                ...
                <x-admin::accordion>
                    ...
                    <x-slot:content>
                        <div class="flex w-full justify-start gap-5">
                            ...
                            <div class="flex flex-col gap-y-1.5">
                                {!! view_render_event('application.admin.sales.order.created_at.before', ['order' => $order]) !!}
                                ...
                                {!! view_render_event('application.admin.sales.order.created_at.after', ['order' => $order]) !!}
                                ...
                                {!! view_render_event('application.admin.sales.order.status_label.after', ['order' => $order]) !!}
                                ...
                                {!! view_render_event('application.admin.sales.order.channel_name.after', ['order' => $order]) !!}
                            </div>
                        </div>
                    </x-slot>
                </x-admin::accordion>
                ...
                <x-admin::accordion>
                    ...
                    <x-slot:content>
                        <div>
                            ...
                            @php $additionalDetails = \Sakura\Payment\Payment::getAdditionalDetails($order->payment->method); @endphp
                            ...
                            {!! view_render_event('application.admin.sales.order.payment-method.after', ['order' => $order]) !!}
                        </div>
                        ...
                        @if ($order->shipping_address)
                            ...
                            {!! view_render_event('application.admin.sales.order.shipping-method.after', ['order' => $order]) !!}
                        @endif
                    </x-slot>
                </x-admin::accordion>
                ...
                {!! view_render_event('application.admin.sales.order.right_component.after', ['order' => $order]) !!}
            </div>
        </div>
    </div>
</x-admin::layouts>
