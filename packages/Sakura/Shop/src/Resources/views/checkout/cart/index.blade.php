...
>
    ...
    {!! view_render_event('application.shop.checkout.cart.header.before') !!}
    ...
    <div class="flex flex-wrap">
        <div class="flex w-full justify-between border border-b border-l-0 border-r-0 border-t-0 px-[60px] py-4 max-lg:px-8 max-md:px-4">
            <div class="flex items-center gap-x-14 max-[1180px]:gap-x-9">
                {!! view_render_event('application.shop.checkout.cart.logo.before') !!}

                <a
                    href="{{ route('shop.home.index') }}"
                    class="flex min-h-[30px]"
                    aria-label="@lang('shop::app.checkout.cart.index.application')"
                >
                <div>
                    <h1 class="title text-black text-xl font-semibold text-center">Akihabara</h1>
                    <h3 class="text-black">Магазин японских сувениров</h3>
                </div>
                    {{-- <img
                        src="{{ core()->getCurrentChannel()->logo_url ?? application_asset('images/logo.svg') }}"
                        alt="{{ config('app.name') }}"
                        width="131"
                        height="29"
                    > --}}
                ...
                {!! view_render_event('application.shop.checkout.cart.logo.after') !!}
            </div>
    ...
    {!! view_render_event('application.shop.checkout.cart.header.after') !!}

    <div class="flex-auto">
        <div class="container px-[60px] max-lg:px-8 max-md:px-4">

            {!! view_render_event('application.shop.checkout.cart.breadcrumbs.before') !!}
            ...
            {!! view_render_event('application.shop.checkout.cart.breadcrumbs.after') !!}
    ...
    @if (core()->getConfigData('sales.checkout.shopping_cart.cross_sell'))
        {!! view_render_event('application.shop.checkout.cart.cross_sell_carousel.before') !!}
        ...
        {!! view_render_event('application.shop.checkout.cart.cross_sell_carousel.after') !!}
    @endif
                ...
                <template v-else>
                    ...
                    >
                        <div class="flex flex-1 flex-col gap-6 max-md:gap-5">
                            ...
                            {!! view_render_event('application.shop.checkout.cart.cart_mass_actions.before') !!}
                            ...
                            {!! view_render_event('application.shop.checkout.cart.cart_mass_actions.after') !!}

                            {!! view_render_event('application.shop.checkout.cart.item.listing.before') !!}
                            ...
                            >
                                <div class="flex justify-between gap-x-2.5 border-b border-zinc-200 pb-5">
                                    <div class="flex gap-x-5">
                                        ...
                                        {!! view_render_event('application.shop.checkout.cart.item_image.before') !!}
                                        ...
                                        {!! view_render_event('application.shop.checkout.cart.item_image.after') !!}

                                        <!-- Cart Item Options Container -->
                                        <div class="grid place-content-start gap-y-2.5 max-md:gap-y-0">
                                            {!! view_render_event('application.shop.checkout.cart.item_name.before') !!}
                                            ...
                                            {!! view_render_event('application.shop.checkout.cart.item_name.after') !!}

                                            {!! view_render_event('application.shop.checkout.cart.item_details.before') !!}
                                            ...
                                            {!! view_render_event('application.shop.checkout.cart.item_details.after') !!}

                                            {!! view_render_event('application.shop.checkout.cart.formatted_total.before') !!}
                                            ...
                                            {!! view_render_event('application.shop.checkout.cart.formatted_total.after') !!}

                                            {!! view_render_event('application.shop.checkout.cart.quantity_changer.before') !!}
                                            ...
                                            {!! view_render_event('application.shop.checkout.cart.quantity_changer.after') !!}
                                        </div>
                                    </div>

                                    <div class="text-right max-md:hidden">
                                        {!! view_render_event('application.shop.checkout.cart.total.before') !!}
                                        ...
                                        {!! view_render_event('application.shop.checkout.cart.total.after') !!}

                                        {!! view_render_event('application.shop.checkout.cart.remove_button.before') !!}
                                        ...
                                        {!! view_render_event('application.shop.checkout.cart.remove_button.after') !!}
                                    </div>
                                </div>
                            </div>

                            {!! view_render_event('application.shop.checkout.cart.item.listing.after') !!}

                            {!! view_render_event('application.shop.checkout.cart.controls.before') !!}

                            <!-- Cart Item Actions -->
                            <div class="flex flex-wrap justify-end gap-8 max-md:justify-between max-md:gap-5">
                                {!! view_render_event('application.shop.checkout.cart.continue_shopping.before') !!}
                                ...
                                {!! view_render_event('application.shop.checkout.cart.continue_shopping.after') !!}

                                {!! view_render_event('application.shop.checkout.cart.update_cart.before') !!}
                                ...
                                {!! view_render_event('application.shop.checkout.cart.update_cart.after') !!}
                            </div>

                            {!! view_render_event('application.shop.checkout.cart.controls.after') !!}
                        </div>

                        {!! view_render_event('application.shop.checkout.cart.summary.before') !!}

                        <!-- Cart Summary Blade File -->
                        @include('shop::checkout.cart.summary')

                        {!! view_render_event('application.shop.checkout.cart.summary.after') !!}
                    </div>
                    ...
                    >
                        <img
                            ...
                            src="{{ application_asset('images/thank-you.png') }}"
