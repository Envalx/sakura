...
>
	...
	{!! view_render_event('application.shop.checkout.onepage.header.before') !!}
        ...
        <div class="flex w-full justify-between border border-b border-l-0 border-r-0 border-t-0 px-[60px] py-4 max-lg:px-8 max-md:px-4">
            <div class="flex items-center gap-x-14 max-[1180px]:gap-x-9">
                <a
                    ...
                    aria-label="@lang('shop::checkout.onepage.index.application')"
                >
				<div>
    ...
    {!! view_render_event('application.shop.checkout.onepage.header.after') !!}

	<!-- Page content -->
	<div class="container mt-8 px-[60px] max-lg:px-8">
		<div class="grid place-items-center gap-y-5 max-md:gap-y-2.5">
			{{ view_render_event('application.shop.checkout.success.image.before', ['order' => $order]) }}
			...
			{{ view_render_event('application.shop.checkout.success.image.after', ['order' => $order]) }}
			...
			{{ view_render_event('application.shop.checkout.success.continue-shopping.before', ['order' => $order]) }}
			...
			{{ view_render_event('application.shop.checkout.success.continue-shopping.after', ['order' => $order]) }}
		</div>
	</div>
</x-shop::layouts>
