@inject ('reviewHelper', 'Sakura\Product\Helpers\Review')
@inject ('productViewHelper', 'Sakura\Product\Helpers\View')
...
@push('meta')
    ...
    @if (core()->getConfigData('catalog.rich_snippets.products.enable'))
        <script type="application/ld+json">
            {!! app('Sakura\Product\Helpers\SEO')->getProductJsonLd($product) !!}
        </script>
    @endif
...
<x-shop::layouts>
    ...
    {!! view_render_event('application.shop.products.view.before', ['product' => $product]) !!}
    ...
    <div class="1180:mt-20">
        <div class="max-1180:hidden">
            ...
            >
                <!-- Description Tab -->
                {!! view_render_event('application.shop.products.view.description.before', ['product' => $product]) !!}
                ...
                {!! view_render_event('application.shop.products.view.description.after', ['product' => $product]) !!}
    ...
    {!! view_render_event('application.shop.products.view.after', ['product' => $product]) !!}
                ...
                >
                    ...
                    <div class="container px-[60px] max-1180:px-0">
                        <div class="mt-12 flex gap-9 max-1180:flex-wrap max-lg:mt-0 max-sm:gap-y-4">
                            ...
                            <!-- Details -->
                            <div
                                class="relative max-w-[590px] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                {!! view_render_event('application.shop.products.name.before', ['product' => $product]) !!}
                                ...
                                {!! view_render_event('application.shop.products.name.after', ['product' => $product]) !!}

                                <!-- Rating -->
                                {!! view_render_event('application.shop.products.rating.before', ['product' => $product]) !!}
                                ...
                                {!! view_render_event('application.shop.products.rating.after', ['product' => $product]) !!}

                                <!-- Pricing -->
                                {!! view_render_event('application.shop.products.price.before', ['product' => $product]) !!}
                                ...
                                @if (\Sakura\Tax\Facades\Tax::isInclusiveTaxProductPrices())
                                    <span class="text-sm font-normal text-zinc-500 max-sm:text-xs">
                                        (@lang('shop::app.products.view.tax-inclusive'))
                                    </span>
                                @endif

                                {!! view_render_event('application.shop.products.short_description.before', ['product' => $product]) !!}

                                <p class="mt-6 text-lg text-zinc-500 max-sm:mt-1.5 max-sm:text-sm">
                                    {!! $product->short_description !!}
                                </p>

                                {!! view_render_event('application.shop.products.short_description.after', ['product' => $product]) !!}
                                ...
                                <div class="mt-8 flex max-w-[470px] gap-4 max-sm:mt-4">
                                    ...
                                    {!! view_render_event('application.shop.products.view.quantity.before', ['product' => $product]) !!}
                                    ...
                                    {!! view_render_event('application.shop.products.view.quantity.after', ['product' => $product]) !!}

                                    @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
                                        <!-- Add To Cart Button -->
                                        {!! view_render_event('application.shop.products.view.add_to_cart.before', ['product' => $product]) !!}

                                        <x-shop::button
                                            type="submit"
                                            class="secondary-button w-full max-w-full px-0 max-md:py-3 max-sm:rounded-lg max-sm:py-1.5 sm:px-2"
                                        ...
                                        {!! view_render_event('application.shop.products.view.add_to_cart.after', ['product' => $product]) !!}
                                    @endif
                                </div>

                                <!-- Buy Now Button -->
                                @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
                                    {!! view_render_event('application.shop.products.view.buy_now.before', ['product' => $product]) !!}
                                    ...
                                    {!! view_render_event('application.shop.products.view.buy_now.after', ['product' => $product]) !!}
                                @endif

                                {!! view_render_event('application.shop.products.view.additional_actions.before', ['product' => $product]) !!}

                                <!-- Share Buttons -->
                                <div
                                    class="mt-10 flex gap-9 max-md:mt-4 max-md:flex-wrap max-sm:justify-center max-sm:gap-3">
                                    {!! view_render_event('application.shop.products.view.compare.before', ['product' => $product]) !!}
                                    ...
                                    {!! view_render_event('application.shop.products.view.compare.after', ['product' => $product]) !!}
                                </div>

                                {!! view_render_event('application.shop.products.view.additional_actions.after', ['product' => $product]) !!}
                            </div>
                        </div>
                    </div>
                </form>
