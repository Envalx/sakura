@if ($product->type == 'bundle')
    {!! view_render_event('application.shop.products.view.bundle-options.before', ['product' => $product]) !!}

    <v-product-bundle-options :errors="errors"></v-product-bundle-options>

    {!! view_render_event('application.shop.products.view.bundle-options.after', ['product' => $product]) !!}
    ...
    @pushOnce('scripts')
        ...
        <script type="module">
            app.component('v-product-bundle-options', {
                ...
                data: function() {
                    return {
                        config: @json(app('Sakura\Product\Helpers\BundleOption')->getBundleConfig($product)),
