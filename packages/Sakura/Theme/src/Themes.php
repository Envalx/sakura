...
use Sakura\Theme\Exceptions\ViterNotFound;
...
class Themes
{
    ...
    public function url(string $filename, ?string $namespace = null)
    {
         ...
         */
        $viters = config('application-vite.viters');
    ...
    /**
     * Set application vite in current theme.
     *
     * @param  mixed  $entryPoints
     * @return mixed
     */
    public function setapplicationVite($entryPoints, ?string $namespace = null)
    {
        /**
         * If the namespace is null, it means the theming system is activated. We use the request URI to
         * detect the theme and provide Vite assets based on the current theme.
         */
        if (empty($namespace)) {
            return $this->current()->setapplicationVite($entryPoints);
        }
